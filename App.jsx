...
function DateRangeFilter({ from, to, onFrom, onTo, onClear }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: "flex", gap: 8 }}>
        <div style={{ flex: 1 }}>
          <input type="date" className="bz-input" style={{ fontSize: 12 }} value={from} max={to || undefined}
            onChange={e => onFrom(e.target.value)} aria-label="From date" />
        </div>
        <div style={{ flex: 1 }}>
          <input type="date" className="bz-input" style={{ fontSize: 12 }} value={to} min={from || undefined}
