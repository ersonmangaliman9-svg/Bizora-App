...
export default function Bizora() {
  ...
  const handleSignedIn = async ({ name, email, token, id }) => {
  ...
  const handleSignedUp = ({ name, email, token, id }) => {
    setSbToken(token || null);
    setSbUserId(id || null);
  ...
  // that redirect.
  useEffect(() => {
    const session = consumeOAuthHashSession();
    if (!session) return;
    (async () => {
      try {
        const user = await sbGetUser(session.access_token);
        ...
        await handleSignedIn({
          ...
          email: user.email, token: session.access_token, id: user.id,
        });
      } catch (e) {
        showToast(e.message || "Couldn't complete Google sign-in. Please try again.");
      }
    })();
...
function SignInScreen({ colors, onSignedIn, goSignUp, goForgot }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await onSignedIn({ name: name || "Business Owner", email: email.trim(), token: data.access_token, id: data.user?.id });
    } catch (e) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (data.access_token && sessionUser?.id) {
        ...
        await onSignedUp({ name, email: email.trim(), token: data.access_token, id: sessionUser.id });
