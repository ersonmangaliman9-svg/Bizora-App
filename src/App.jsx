...
/* ---------------- Data backup / restore ----------------
   Lets an owner download everything (business profile, products,
   customers, sales, expenses, invoices) as a single JSON file, and load
   that same file back in later — on this device or a new one. This is a
   local export, separate from the automatic cloud sync. */
const BACKUP_FORMAT = "bizora-backup";
const BACKUP_VERSION = 1;

const buildBackup = ({ business, products, customers, sales, expenses, invoices }) => ({
  format: BACKUP_FORMAT,
  version: BACKUP_VERSION,
  exportedAt: new Date().toISOString(),
  business, products, customers, sales, expenses, invoices,
});

// On phones — and especially on an installed/standalone PWA — a hidden
// <a download> click on a blob: URL is very often silently swallowed by
// the browser instead of saving a file (the JSON just opens in a new tab,
// or nothing visibly happens at all). The Web Share API's file sharing,
// supported by most modern mobile browsers, hands the file to the OS share
// sheet instead so the person can actually pick "Save to Files" / Drive /
// etc. We try that first and only fall back to the classic anchor-click
// download (which is what desktop browsers want) if sharing isn't
// available or isn't supported for this file.
const downloadBackupFile = async (backup) => {
  const stamp = todayISO();
  const bizSlug = (backup.business?.name || "bizora").trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "bizora";
  const filename = `${bizSlug}-backup-${stamp}.json`;
  ...
  if (typeof navigator !== "undefined" && navigator.share && navigator.canShare) {
    try {
      ...
      // real failure, so don't fall through to also trigger a download.
      if (err && err.name === "AbortError") return { filename, method: "cancelled" };
      // Any other error (e.g. share not actually supported): fall back below.
    }
  }
  ...
  return { filename, method: "anchor" };
};

// Reads + validates a chosen backup file, throwing a friendly error message
// on anything that doesn't look like a Bizora backup.
const parseBackupFile = (file) =>
  new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (!json || typeof json !== "object" || json.format !== BACKUP_FORMAT) {
        reject(new Error("That file isn't a Bizora backup."));
        return;
      }
      resolve({
...
const SUPABASE_ANON_KEY = "sb_publishable_VkmSJ9SveKghDmiQxuFR0Q_XRXKTcXC";
...
async function sbAuth(path, body) {
  ...
  if (!res.ok) throw new Error(`${data.error_description || data.msg || data.error || "Something went wrong. Please try again."} (status ${res.status})`);
  return data;
}
...
async function sbGetUser(token) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(`${data.msg || data.error_description || data.error || "Couldn't verify that Google sign-in."} (status ${res.status})`);
  return data;
}

async function sbRest(path, { method = "GET", token, body, prefer } = {}) {
  const headers = {
...
const seedData = () => {
  ...
  // Guarantee a handful of products start below their reorder point, so a
  // freshly-loaded demo account always has something to show in the
  // low-stock alerts / Notifications feature instead of leaving it up to
  // chance whether any product happened to land there.
  const lowStockCandidates = products.filter(p => p.category !== "Load & Cards");
  const forcedLowStock = [lowStockCandidates[2], lowStockCandidates[7], lowStockCandidates[13]].filter(Boolean);
  forcedLowStock.forEach(p => { p.stock = Math.max(0, p.minStock - 1 - Math.floor(Math.random() * 3)); });
  ...
  for (let i = DAYS - 1; i >= 0; i--) {
    ...
    for (let t = 0; t < txCount; t++) {
      ...
      // Regulars occasionally buy on utang (credit) — some fully unpaid,
      // some partially paid — so the Utang/Credit report isn't empty in
      // the demo. Walk-in sales are always paid in full.
      let amountPaid = total;
      if (useRegular && Math.random() < 0.22) {
        amountPaid = Math.random() < 0.5 ? 0 : Math.round(total * (0.2 + Math.random() * 0.5));
      }
      const sale = {
        id: uid(), date: iso, productId: p.id, productName: p.name, qty,
        unitPrice: p.sellingPrice, total, customerId: customer.id,
        payment: PAYMENT_METHODS[Math.floor(Math.random() * PAYMENT_METHODS.length)].id,
        notes: amountPaid < total ? "Utang" : "", invoiced: false, amountPaid,
      };
  ...
  // Guarantee a couple of overdue utang entries too, for the same reason —
  // pick two regulars and give each an unpaid sale from well over a week
  // ago, safely past the alert threshold, so Notifications always has
  // something real to show right after a demo load.
  const overdueCustomers = regulars.slice(0, Math.min(2, regulars.length));
  overdueCustomers.forEach((c, idx) => {
    const p = products[(idx * 5) % products.length];
    const daysAgo = 6 + idx * 4; // 6, 10 days old
    ...
    sales.push({
      id: uid(), date: iso, productId: p.id, productName: p.name, qty,
      unitPrice: p.sellingPrice, total, customerId: c.id,
      payment: PAYMENT_METHODS[0].id, notes: "Utang", invoiced: false, amountPaid: 0,
    });
  });
...
/* ---- Bizora Mascot: "Pesoy", a friendly peso-coin buddy for the Dashboard.
   Same coin-and-crossbar DNA as BizoraMark, given a face, arms, and legs so
   it reads as a character rather than a logo. Tap it for a fresh tip. ---- */
// Shown only to brand-new accounts with nothing recorded yet (see
// buildMascotReport / hasData in Dashboard) — a short, ordered "what to do
// first" checklist rather than random flavor text, so the mascot bubble
// actively helps a new owner get set up instead of just being cute.
const MASCOT_TIPS = [
  "👋 Kumusta! Let's get your store set up — start by adding your first product.",
  "Tap the Stock tab, then + Add. Give it a name and price — or pick straight from our grocery catalog.",
  "Got products in? Tap the + button anytime to record your first sale.",
  "Don't forget expenses too — restock, rent, bills — para accurate ang profit mo.",
  "Add your regular customers so you can track utang/credit easily later.",
  "Upload your GCash or Maya QR in Settings so customers can pay you faster.",
  "Explore the More tab anytime for reports, invoices, and customer records.",
];

// Turns the owner's actual numbers into a short rotating "report" the
// mascot can read out — today's sales/expenses/profit, low-stock and utang
// alerts, best seller, and month-to-date totals. Only facts that are
// actually true get included (e.g. no low-stock line if nothing's low), so
// the list naturally shortens or grows with the business itself.
function buildMascotReport({
  todaySalesCount, todaySalesTotal, todayExpCount, todayExpTotal, todayProfit,
  monthRevenue, monthProfit, lowStock, alerts, bestSellers, productsCount, customersCount,
}) {
  const msgs = [];

  msgs.push(
    todaySalesCount > 0
      ? `Today so far: ${todaySalesCount} sale${todaySalesCount === 1 ? "" : "s"} · ${peso(todaySalesTotal)} in revenue 💰`
      : "No sales logged yet today — tap + to record your first one!"
  );

  msgs.push(
    todayProfit >= 0
      ? `Today's profit is at ${peso(todayProfit)} so far. Keep it up, boss! 📈`
      : `Heads up: today's expenses are ahead of sales by ${peso(Math.abs(todayProfit))}.`
  );

  msgs.push(
    todayExpCount > 0
      ? `You've logged ${todayExpCount} expense${todayExpCount === 1 ? "" : "s"} today, totaling ${peso(todayExpTotal)}.`
      : "Wag kalimutan i-log ang gastos ngayon — keep those numbers honest."
  );

  if (lowStock && lowStock.length > 0) {
    const names = lowStock.slice(0, 2).map(p => p.name).join(", ");
    msgs.push(`Low stock alert: ${lowStock.length} product${lowStock.length === 1 ? "" : "s"} running low (${names}${lowStock.length > 2 ? ", …" : ""}) 🔔`);
  }

  const utangAlerts = (alerts || []).filter(a => a.kind === "utang");
  if (utangAlerts.length > 0) {
    msgs.push(`${utangAlerts.length} customer${utangAlerts.length === 1 ? "" : "s"} still owe${utangAlerts.length === 1 ? "s" : ""} you money — check Utang/Credit.`);
  }

  if (alerts && alerts.length > 0) {
    msgs.push(`You have ${alerts.length} notification${alerts.length === 1 ? "" : "s"} needing attention — tap the bell 🔔`);
  }

  if (bestSellers && bestSellers.length > 0 && bestSellers[0].product) {
    msgs.push(`${bestSellers[0].product.name} is your best seller — ${bestSellers[0].qty} sold so far!`);
  }

  if (monthRevenue > 0) {
    msgs.push(`This month: ${peso(monthRevenue)} in sales, ${peso(monthProfit)} profit.`);
  }

  if (productsCount > 0) {
    msgs.push(`You're tracking ${productsCount} product${productsCount === 1 ? "" : "s"} in your inventory.`);
  }
  if (customersCount > 0) {
    msgs.push(`You have ${customersCount} customer${customersCount === 1 ? "" : "s"} on file.`);
  }

  return msgs;
}

function BizoraMascot({ size = 60, messages }) {
  // A real data report (see buildMascotReport) takes priority; a brand-new
  // account with nothing on file yet falls back to the generic tips.
  const hasReport = Array.isArray(messages) && messages.length > 0;
  const source = hasReport ? messages : MASCOT_TIPS;
  // Kept in a ref (not state) so the interval below always reads the
  // latest list without needing to tear down and restart every time the
  // owner's data — and therefore the report — changes.
  const sourceRef = useRef(source);
  sourceRef.current = source;

  const [wave, setWave] = useState(0);
  // Both the real-data report and the new-user checklist read best in a
  // fixed order (a briefing / a step-by-step list), so always start at the
  // first message rather than a random one.
  ...
  const startAutoSlide = () => {
    ...
    autoTimerRef.current = setInterval(() => {
      const len = sourceRef.current.length || 1;
      setTipIndex(i => (i + 1) % len);
    }, 3000);
  };

  // Auto-advance to the next message every 3 seconds, looping back to the
  // start. Set up once on mount; startAutoSlide always reads the current
  // list via sourceRef, so it stays correct even if the report changes
  // length later without needing to restart the timer.
  useEffect(() => {
    startAutoSlide();
  ...
  const poke = () => {
    setWave(w => w + 1);
    setTipIndex(i => (i + 1) % (sourceRef.current.length || 1));
    // Restart the 3s auto-slide from this moment, so a tap doesn't get
    // immediately overridden by a timer that was already mid-countdown.
    startAutoSlide();
  };

  // Clamp defensively in case the report shrank (e.g. an alert just got
  // resolved) between renders, leaving a stale index out of range.
  const safeIndex = source.length ? tipIndex % source.length : 0;
  const tip = source[safeIndex];

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <button onClick={poke} title="Kumusta! Tap me 👋" style={{ background: "none", border: "none", padding: 0, cursor: "pointer", flexShrink: 0, lineHeight: 0 }}>
        <div key={wave} style={{ animation: wave ? "bzMascotBounce .7s cubic-bezier(.34,1.56,.64,1)" : "none" }}>
          <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <radialGradient id="mascotGrad" cx="38%" cy="32%" r="75%">
                <stop offset="0" stopColor="#22B192" />
                <stop offset="0.6" stopColor="#14876F" />
                <stop offset="1" stopColor="#0A5548" />
              </radialGradient>
              <linearGradient id="mascotLimb" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0" stopColor="#1CA383" />
                <stop offset="1" stopColor="#0E6E5C" />
              </linearGradient>
            </defs>

            {/* ground shadow */}
            <ellipse cx="50" cy="92.5" rx="19" ry="3.5" fill="#000" opacity="0.1" />

            {/* legs */}
            <rect x="38.5" y="78" width="7" height="13" rx="3.5" fill="url(#mascotLimb)" />
            <rect x="54.5" y="78" width="7" height="13" rx="3.5" fill="url(#mascotLimb)" />

            {/* arms */}
            <path d="M25 58 Q13 62 16 72" stroke="url(#mascotLimb)" strokeWidth="7" strokeLinecap="round" fill="none" />
            <path d="M75 58 Q90 50 86 34" stroke="url(#mascotLimb)" strokeWidth="7" strokeLinecap="round" fill="none"
                  style={{ transformOrigin: "76px 57px", animation: wave ? "bzMascotWave .7s ease" : "none" }} />

            {/* coin body */}
            <circle cx="50" cy="51" r="34" fill="url(#mascotGrad)" />
            <circle cx="50" cy="51" r="34" fill="none" stroke="#0A5548" strokeWidth="1.5" opacity="0.5" />
            <circle cx="50" cy="51" r="29.5" fill="none" stroke="#F6F4EE" strokeOpacity="0.18" strokeWidth="2" strokeDasharray="1 4.4" />

            {/* peso mark, softened */}
            <text x="50" y="64.5" textAnchor="middle" fontFamily="Sora, sans-serif" fontWeight="800" fontSize="24" fill="#E3A23C" opacity="0.95">₱</text>

            {/* face: big friendly eyes */}
            <circle cx="39" cy="45" r="6.4" fill="#F6F4EE" />
            <circle cx="61" cy="45" r="6.4" fill="#F6F4EE" />
            <circle cx="40.6" cy="46.4" r="3.1" fill="#16241F" />
            <circle cx="62.6" cy="46.4" r="3.1" fill="#16241F" />
            <circle cx="39.2" cy="44.4" r="1.15" fill="#fff" />
            <circle cx="61.2" cy="44.4" r="1.15" fill="#fff" />

            {/* soft eyebrow hint for warmth, not stern */}
            <path d="M34 38 Q39 35.5 44 37.5" stroke="#0A5548" strokeWidth="1.6" strokeLinecap="round" fill="none" opacity="0.55" />
            <path d="M56 37.5 Q61 35.5 66 38" stroke="#0A5548" strokeWidth="1.6" strokeLinecap="round" fill="none" opacity="0.55" />

            {/* blush */}
            <circle cx="31" cy="55" r="3.6" fill="#E3A23C" opacity="0.4" />
            <circle cx="69" cy="55" r="3.6" fill="#E3A23C" opacity="0.4" />

            {/* big warm smile */}
            <path d="M39 55 Q50 66 61 55" stroke="#F6F4EE" strokeWidth="3.2" strokeLinecap="round" fill="none" />
          </svg>
        </div>
      </button>
      <div key={tipIndex} style={{ position: "relative", background: "var(--surface2)", borderRadius: 14, padding: "9px 13px", fontSize: 12, fontWeight: 600, color: "var(--text)", flex: 1, animation: "bzfade .3s ease" }}>
        {tip}
        <div style={{ position: "absolute", left: -6, top: "50%", transform: "translateY(-50%)", width: 0, height: 0, borderTop: "6px solid transparent", borderBottom: "6px solid transparent", borderRight: "6px solid var(--surface2)" }} />
      </div>
    </div>
  );
}
...
/* ---------------- New-user tutorial ----------------
   ...
   in the main component). Replayable any time from Settings. Ordered to
   match the actual order a new owner would use the app in — add a
   product, record a sale, log an expense — so each step is something they
   can go do immediately after closing the tour, not just background info. */
const TUTORIAL_STEPS = [
  { icon: Sparkles, title: "Welcome to Bizora!", desc: "Your all-in-one business partner for tracking sales, expenses, and inventory — built for Filipino entrepreneurs. Let's get your store set up in under a minute.", accent: "#E3A23C" },
  { icon: Boxes, title: "Add Your First Product", desc: "Open the Stock tab and tap + Add. Give it a name, purchase price, and selling price — or pick straight from our built-in grocery catalog to save time.", accent: "#14876F" },
  { icon: ShoppingCart, title: "Record Your First Sale", desc: "Tap the floating + button any time you make a sale. Choose the product and quantity — stock and profit update automatically the moment you save.", accent: "#2E9E6C" },
  { icon: Wallet, title: "Log Every Expense", desc: "Restocking costs, rent, bills — add them under the Expenses tab so your profit numbers stay accurate, not just your sales.", accent: "#D6584A" },
  { icon: Home, title: "Your Dashboard, At a Glance", desc: "Today's sales, profit, and low-stock alerts are all here the moment you open the app — no more digging through notebooks or spreadsheets.", accent: "#0E6E5C" },
  { icon: BarChart2, title: "Customers, Utang & Reports", desc: "Head to the More tab to manage customers, track utang/credit, generate invoices, and view deeper sales reports whenever you need them.", accent: "#0A5548" },
  { icon: Check, title: "You're All Set!", desc: "That's the whole tour. Go add your first product or record a sale now — Bizora handles the rest.", accent: "#E3A23C" },
];
...
export default function Bizora() {
  ...
  const [moreScreen, setMoreScreen] = useState(null); // customers | utang | invoices | reports | settings | upgrade
  const [openUtangCustomer, setOpenUtangCustomer] = useState(null);
  ...
  // Reset the parallax offset whenever the user leaves the Dashboard, so it
  // doesn't reopen mid-parallax next time (scrollY stops updating for other
  // tabs now, so it would otherwise be left stale from the last visit).
  useEffect(() => {
    if (tab !== "dashboard") setScrollY(0);
  }, [tab]);
  ...
  // Load — device-local preferences, business profile, and POS working
  // data are cached here (and mirrored to Supabase's business_data table
  // when signed in), so profile edits and restored backups survive a
  // reload instead of only living in memory. Identity and Premium status
  // still come fresh from Supabase's profiles table on every login, since
  // that must work for customers who have no Claude account at all — this
  // effect just merges in whatever fuller business data (QR code, label,
  // etc.) is sitting in the business_data row on top of it.
  //
  ...
  useEffect(() => {
    ...
    (async () => {
      let cloudRow = null;
      // Cloud is the source of truth so the same data follows the account
      // across logout/login and across devices. Only look it up once we
      // actually have a Supabase session for this account.
      if (sbUserId && sbToken) {
        try {
          const rows = await sbRest(`/business_data?user_id=eq.${sbUserId}&select=data`, { token: sbToken });
          if (rows && rows[0]) cloudRow = rows[0].data;
        } catch (e) { /* fall through to local copy below */ }
      }
      if (cloudRow) {
        setTheme(cloudRow.theme || "light");
        setNotifyLowStock(cloudRow.notifyLowStock !== false);
        setNotifyPush(cloudRow.notifyPush === true);
        // Merge (not overwrite) — the profiles-table lookup in
        // handleSignedIn already put the basics (name/type/avatar) on
        // business, but this is the fuller copy (also carries QR code,
        // label, etc., and reflects anything restored from a backup), so
        // it should win field-for-field wherever it has data.
        if (cloudRow.business) setBusiness(prev => ({ ...(prev || {}), ...cloudRow.business }));
        setProducts(cloudRow.products || []);
        setCustomers(cloudRow.customers || []);
        setSales(cloudRow.sales || []);
        setExpenses(cloudRow.expenses || []);
        setInvoices(cloudRow.invoices || []);
        setLoaded(true);
        return;
      }
      try {
        ...
        if (res && res.value) {
          ...
          setNotifyPush(d.notifyPush === true);
          ...
          // Migrate this device's local-only copy up to Supabase so it
          // isn't stranded on this browser the next time they log in
          // elsewhere.
          if (sbUserId && sbToken) {
            sbRest(`/business_data?on_conflict=user_id`, {
              method: "POST", token: sbToken, prefer: "resolution=merge-duplicates",
              body: { user_id: sbUserId, data: d, updated_at: new Date().toISOString() },
            }).catch(() => {});
          }
        } else {
          setProducts([]);
  ...
  }, [authUser?.email, sbUserId, sbToken]);

  // Save (debounced) — writes to this device's local cache AND to
  // Supabase (business_data table), keyed by the signed-in account, so the
  // same products/sales/expenses/etc. follow the account across logout,
  // login, and different devices instead of staying stuck on one browser.
  useEffect(() => {
    if (!loaded || !authUser?.email) return;
    const key = dataKeyFor(authUser.email);
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      const payload = { theme, notifyLowStock, notifyPush, business, products, customers, sales, expenses, invoices };
      try {
        await window.storage.set(key, JSON.stringify(payload), false);
      } catch (e) { /* silent */ }
      if (sbUserId && sbToken) {
        try {
          await sbRest(`/business_data?on_conflict=user_id`, {
            method: "POST", token: sbToken, prefer: "resolution=merge-duplicates",
            body: { user_id: sbUserId, data: payload, updated_at: new Date().toISOString() },
          });
        } catch (e) { /* silent — local copy above still saved */ }
      }
    }, 400);
  }, [loaded, authUser?.email, sbUserId, sbToken, theme, notifyLowStock, notifyPush, business, products, customers, sales, expenses, invoices]);
  ...
  // Ask the browser for notification permission the moment the user turns
  // the setting on (never on load/unprompted). Only flips notifyPush to
  // true if permission is actually granted, so the toggle never lies.
  //
  // Also registers the service worker here (not just permission). Most
  // mobile browsers — Android Chrome in particular — refuse to run
  // `new Notification()` from page JS at all (silent "Illegal constructor"
  // error) and only allow notifications through a registered service
  // worker's `showNotification()`. Without this registration, notifyPush
  // would happily flip to "on" and never actually show anything on phones.
  const requestPushPermission = async () => {
    if (typeof window === "undefined" || !("Notification" in window)) {
      showToast("Push notifications aren't supported on this device/browser.");
      return false;
    }
    ...
    if ("serviceWorker" in navigator) {
      try {
        ...
        // Registration failing (e.g. served over plain http, or an
        // unsupported browser) just means we'll fall back to the direct
        // Notification() constructor later — not a hard failure here.
      }
    }
    return true;
  };
  const handleTogglePush = async (next) => {
    ...
    if (ok) { setNotifyPush(true); showToast("Push notifications enabled"); }
  };

  // notifyPush can also arrive already-true from a synced profile (e.g. it
  // was turned on on another device) — in that case the toggle-triggered
  // registration above never runs on *this* device/browser. Make sure the
  // service worker is registered here too, whenever permission already
  // shows "granted", so showNotification() has something to call.
  useEffect(() => {
  ...
  // Heartbeat — pings `last_seen` on this account's profile row every ~60s
  // while the app is open (plus once immediately on login), so the Admin
  // panel can show who's online and when everyone else was last active.
  // Best-effort only: a failed ping (offline, expired session) just waits
  // for the next tick rather than showing an error, since this is purely
  // informational for the admin and shouldn't interrupt the signed-in user.
  const heartbeatRef = useRef(null);
  useEffect(() => {
    if (!authed || !sbToken || !sbUserId) return;
    const ping = () => {
      sbRest(`/profiles?id=eq.${sbUserId}`, {
        method: "PATCH", token: sbToken, body: { last_seen: new Date().toISOString() },
      }).catch(() => { /* offline or session expired — try again next tick */ });
    };
    ping();
    heartbeatRef.current = setInterval(ping, 60000);
    return () => clearInterval(heartbeatRef.current);
  }, [authed, sbToken, sbUserId]);
  ...
  const todayExpCount = todayExpenses.length;
  ...
  // Customers with unpaid utang that's sat unpaid for a few days — flagged
  // the same way low-stock items are, so both show up together as alerts.
  const UTANG_ALERT_DAYS = 3;
  const overdueUtang = useMemo(() => {
    return customers.map(c => {
      const unpaid = sales.filter(s => s.customerId === c.id && (s.total - (s.amountPaid ?? s.total)) > 0);
      if (!unpaid.length) return null;
      const oldestDate = unpaid.reduce((a, s) => (s.date < a ? s.date : a), unpaid[0].date);
      const daysOld = Math.floor((Date.now() - new Date(oldestDate).getTime()) / 86400000);
      if (daysOld < UTANG_ALERT_DAYS) return null;
      const balance = unpaid.reduce((a, s) => a + (s.total - (s.amountPaid ?? s.total)), 0);
      return { customer: c, balance, daysOld };
    }).filter(Boolean).sort((a, b) => b.daysOld - a.daysOld);
  }, [customers, sales]);

  // Unified alert feed — low-stock items + overdue utang — surfaced via the
  // notification bell, the "More" menu, and (if opted in) a native push.
  const alerts = useMemo(() => {
    if (!notifyLowStock && isPremium) return [];
    const utangAlerts = overdueUtang.map(u => ({
      id: `utang-${u.customer.id}`, kind: "utang",
      title: `${u.customer.name} has unpaid utang`,
      subtitle: `${peso(u.balance)} owed for ${u.daysOld} day${u.daysOld === 1 ? "" : "s"}`,
      customerId: u.customer.id,
    }));
    const stockAlerts = lowStock.map(p => ({
      id: `stock-${p.id}`, kind: "stock",
      title: `${p.name} is running low`,
      subtitle: `${p.stock} left · restock below ${p.minStock}`,
      productId: p.id,
    }));
    return [...utangAlerts, ...stockAlerts];
  }, [overdueUtang, lowStock, notifyLowStock, isPremium]);

  // Best-effort "push" — fires a native browser/OS notification for active
  // alerts (low stock, overdue utang) once per account per day, so the
  // owner gets nudged even without opening the app to the dashboard.
  // There's no backend push service here, so this only fires while the
  // app happens to be open/loaded; it's a lightweight stand-in, not true
  // background push.
  useEffect(() => {
    if (!loaded || !notifyPush || alerts.length === 0) return;
    if (typeof window === "undefined" || !("Notification" in window)) return;
    if (Notification.permission !== "granted") return;
    const throttleKey = `bizora-push-last-${authUser?.email || "guest"}`;
    const today2 = todayISO();
    let last = null;
    try { last = window.localStorage.getItem(throttleKey); } catch (e) {}
    if (last === today2) return;

    const body = alerts.length === 1 ? alerts[0].title : `${alerts.length} things need your attention today`;
    const opts = { body, tag: "bizora-alerts" };

    // Most mobile browsers (Android Chrome especially) throw on
    // `new Notification()` and only allow notifications via a service
    // worker's showNotification(). Prefer that path when a worker is
    // registered; fall back to the direct constructor for desktop
    // browsers that don't need one.
    const fireViaServiceWorker = async () => {
      if (!("serviceWorker" in navigator)) return false;
      try {
        const reg = await navigator.serviceWorker.getRegistration();
        if (!reg) return false;
        await reg.showNotification("Bizora", opts);
        return true;
      } catch (e) { return false; }
    };

    (async () => {
      const shown = await fireViaServiceWorker();
      if (!shown) {
        try {
          const n = new Notification("Bizora", opts);
          n.onclick = () => { try { window.focus(); } catch (e) {} };
        } catch (e) { /* silent — e.g. blocked by browser this session */ }
      }
      try { window.localStorage.setItem(throttleKey, today2); } catch (e) {}
    })();
  }, [loaded, notifyPush, alerts, authUser?.email]);
  ...
  const addProduct = (p) => { setProducts(prev => [{ ...p, id: uid(), createdAt: p.createdAt || todayISO() }, ...prev]); showToast("Product added ✓"); };
  const addProductsBulk = (list) => {
    setProducts(prev => {
      ...
      const fresh = list.filter(it => !existingNames.has(it.name.toLowerCase()))
        .map(it => ({ id: uid(), name: it.name, sku: "", category: it.category, purchasePrice: it.purchasePrice, sellingPrice: it.sellingPrice, stock: 0, minStock: 5, image: "", createdAt: todayISO() }));
  ...
  const restockProductQty = (product, qty, cost, note) => {
    setProducts(prev => prev.map(p => p.id === product.id ? { ...p, stock: p.stock + qty, lastRestockedAt: todayISO() } : p));
    if (cost > 0) {
  ...
  // Applies a payment against a customer's oldest unpaid sales first (FIFO)
  // until the amount is exhausted, updating each sale's amountPaid.
  const recordPayment = (customerId, amount) => {
    let remaining = amount;
    setSales(prev => {
      const sorted = [...prev].filter(s => s.customerId === customerId).sort((a, b) => a.date.localeCompare(b.date));
      const updates = {};
      for (const s of sorted) {
        if (remaining <= 0) break;
        const paid = s.amountPaid ?? s.total;
        const owed = s.total - paid;
        if (owed <= 0) continue;
        const applied = Math.min(owed, remaining);
        updates[s.id] = paid + applied;
        remaining -= applied;
      }
      return prev.map(s => updates[s.id] !== undefined ? { ...s, amountPaid: updates[s.id] } : s);
    });
    showToast(`Payment of ${peso(amount)} recorded ✓`);
  };
  ...
  return (
    <div className="bz-frame" style={{
      ...
      <style>{`
        ...
        @keyframes bzMascotBounce { 0% { transform: translateY(0) scale(1); } 30% { transform: translateY(-8px) scale(1.05, .96); } 55% { transform: translateY(0) scale(.97, 1.04); } 80% { transform: translateY(-2px) scale(1.01); } 100% { transform: translateY(0) scale(1); } }
        @keyframes bzMascotWave { 0% { transform: rotate(0deg); } 20% { transform: rotate(-22deg); } 40% { transform: rotate(12deg); } 60% { transform: rotate(-18deg); } 80% { transform: rotate(8deg); } 100% { transform: rotate(0deg); } }
        * { box-sizing: border-box; }
      `}</style>

      <div ref={scrollRef} className="bz-scroll" style={{ height: "100svh", overflowY: "auto", paddingBottom: 88 }}
        onScroll={(e) => {
          // scrollY only drives the Dashboard header's parallax effect, but
          // this handler used to call setScrollY on every scroll pixel
          // regardless of which tab was open. Since scrollY lives on the
          // root App component, that forced the *entire* tree — every card
          // on Sales/Inventory/Expenses, each with its own Reveal animation
          // — to re-render on every single scroll event, which is what made
          // scrolling those lists feel heavy/sticky instead of smooth.
          // Only track it while Dashboard is actually visible, and batch it
          // to one update per animation frame so it never fights the
          // browser's native scrolling.
          if (tab !== "dashboard") return;
          const top = e.currentTarget.scrollTop;
          if (scrollRafRef.current) cancelAnimationFrame(scrollRafRef.current);
          scrollRafRef.current = requestAnimationFrame(() => setScrollY(top));
        }}>
        ...
        {tab === "dashboard" && (
          <Dashboard {...{ business, authUser, colors, theme, setTheme, scrollY, todaySalesTotal, todayExpTotal, todayProfit,
            todaySalesCount, todayExpCount, monthRevenue, monthProfit, products, lowStock, recentTx, customers, bestSellers, chartData, isPremium, notifyLowStock, alerts,
            goUpgrade: () => { setTab("more"); setMoreScreen("upgrade"); },
            goNotifications: () => { setTab("more"); setMoreScreen("notifications"); } }} />
        )}
        {tab === "sales" && (
        ...
        {tab === "more" && !moreScreen && (
          <MoreMenu {...{ colors, isPremium, isAdmin, setMoreScreen, business, authUser, alertsCount: alerts.length }} />
        )}
        {tab === "more" && moreScreen === "notifications" && (
          <NotificationsScreen {...{ alerts, colors, onBack: () => setMoreScreen(null),
            onOpenStock: () => { setTab("inventory"); setMoreScreen(null); },
            onOpenUtang: (customerId) => { setOpenUtangCustomer(customerId); setMoreScreen("utang"); } }} />
        )}
        {tab === "more" && moreScreen === "customers" && (
          <CustomersScreen {...{ customers, sales, colors, onAdd: () => setShowAddCustomer(true), onDelete: deleteCustomer, onBack: () => setMoreScreen(null),
            goUtang: (customerId) => { setOpenUtangCustomer(customerId); setMoreScreen("utang"); } }} />
        )}
        {tab === "more" && moreScreen === "utang" && (
          <UtangScreen {...{ customers, sales, colors, onBack: () => { setMoreScreen(null); setOpenUtangCustomer(null); },
            onRecordPayment: recordPayment, openCustomerId: openUtangCustomer }} />
        )}
        ...
        {tab === "more" && moreScreen === "reports" && (
          <ReportsScreen {...{ sales, expenses, products, customers, colors, isPremium, business, showToast, onBack: () => setMoreScreen(null), goUpgrade: () => setMoreScreen("upgrade") }} />
        )}
        {tab === "more" && moreScreen === "settings" && (
          <SettingsScreen {...{ business, setBusiness, theme, setTheme, colors, isPremium, authUser, onBack: () => setMoreScreen(null), goUpgrade: () => setMoreScreen("upgrade"),
            products, customers, sales, expenses, invoices, setProducts, setCustomers, setSales, setExpenses, setInvoices,
            onLogout: () => {
            ...
            showToast, notifyLowStock, setNotifyLowStock, notifyPush, onTogglePush: handleTogglePush }} />
        )}
...
/* Compact "From / To" date range picker with quick presets. Used by Sales,
   Expenses, and Inventory to filter records by date. `from`/`to` are ISO
   date strings ("" = no bound). */
function DateRangeFilter({ from, to, onFrom, onTo, onClear }) {
  const active = !!(from || to);
  const preset = (days) => {
    const end = new Date();
    const start = new Date();
    if (days !== null) start.setDate(end.getDate() - (days - 1));
    onFrom(days === null ? "" : start.toISOString().slice(0, 10));
    onTo(days === null ? "" : end.toISOString().slice(0, 10));
  };
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: "flex", gap: 8 }}>
        {/* minWidth: 0 is the fix — flex items default to min-width: auto,
           and a native <input type="date"> has a fairly wide intrinsic
           content width it won't shrink below. On the app's narrow frame
           that pushed this row wider than its container, and since the
           frame clips overflow, the "To" box was being cut off entirely
           instead of sitting beside "From". Letting both items shrink to
           their allotted half keeps both dates visible side by side. */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <input type="date" className="bz-input" style={{ fontSize: 12, minWidth: 0, padding: "11px 8px" }} value={from} max={to || undefined}
            onChange={e => onFrom(e.target.value)} aria-label="From date" />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <input type="date" className="bz-input" style={{ fontSize: 12, minWidth: 0, padding: "11px 8px" }} value={to} min={from || undefined}
            onChange={e => onTo(e.target.value)} aria-label="To date" />
        </div>
      </div>
      <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
        {[["7D", 7], ["30D", 30], ["90D", 90]].map(([label, days]) => (
          <button key={label} onClick={() => preset(days)} style={{
        ...
        {active && (
          <button onClick={onClear} style={{
            border: "none", background: "none", color: "var(--primary)", fontSize: 11, fontWeight: 700, cursor: "pointer", padding: "4px 6px",
          }}>Clear dates</button>
        )}
      </div>
    </div>
...
function Dashboard(props) {
  const { business, authUser, theme, setTheme, todaySalesTotal, todayExpTotal, todayProfit,
    todaySalesCount, todayExpCount, monthRevenue, monthProfit,
    products, lowStock, recentTx, customers, bestSellers, chartData, isPremium, notifyLowStock, alerts, goUpgrade, goNotifications } = props;
  ...
  // A returning owner with real data on file gets a rotating "report" from
  // the mascot instead of generic onboarding tips — actual sales, expenses,
  // stock, and notification counts, so the bubble reads like a quick daily
  // briefing rather than random flavor text. A brand-new account (nothing
  // recorded anywhere yet) still gets the friendly generic tips, since a
  // report of all-zeros isn't useful to anyone.
  const hasData = products.length > 0 || customers.length > 0 || (recentTx && recentTx.length > 0)
    || todaySalesCount > 0 || todayExpCount > 0 || monthRevenue > 0;
  const mascotReport = useMemo(() => (
    hasData
      ? buildMascotReport({
          todaySalesCount, todaySalesTotal, todayExpCount, todayExpTotal, todayProfit,
          monthRevenue, monthProfit, lowStock, alerts, bestSellers,
          productsCount: products.length, customersCount: customers.length,
        })
      : null
  ), [hasData, todaySalesCount, todaySalesTotal, todayExpCount, todayExpTotal, todayProfit,
      monthRevenue, monthProfit, lowStock, alerts, bestSellers, products.length, customers.length]);
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 18px 4px" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={goNotifications} style={{ position: "relative", background: "var(--surface2)", border: "none", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--text)" }}>
            <Bell size={16} />
            {alerts && alerts.length > 0 && (
              <div style={{ position: "absolute", top: -3, right: -3, minWidth: 16, height: 16, padding: "0 3px", borderRadius: 999, background: "var(--danger)", color: "#fff", fontSize: 9, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>
      ...
      {/* Mascot */}
      <Reveal>
        <div className="bz-card" style={{ margin: "14px 18px 0", padding: "12px 14px" }}>
          <BizoraMascot messages={mascotReport} />
        </div>
      </Reveal>
...
function SalesScreen({ sales, products, customers, colors, onInvoice, invoices }) {
  ...
  const filtered = sorted
    .filter(s => s.productName.toLowerCase().includes(filter.toLowerCase()))
  ...
  return (
    <div>
      ...
      <div style={{ padding: "6px 18px 90px" }}>
        {filtered.length === 0 ? (
          <EmptyState icon={ShoppingCart} title="No sales found"
            subtitle={filter || dateFrom || dateTo ? "Try a different search or date range." : "Tap the + button to record a sale."} />
        ) : filtered.map((s, i) => {
          const c = customers.find(cu => cu.id === s.customerId);
...
function InventoryScreen({ products, colors, isPremium, onAdd, onEdit, onDelete, onRestock, goUpgrade }) {
  ...
  const filtered = products
    ...
    .filter(p => !dateFrom || (p.createdAt || "9999-99-99") >= dateFrom)
    .filter(p => !dateTo || (p.createdAt || "0000-00-00") <= dateTo)
    .filter(p => !lowOnly || p.stock <= p.minStock);
  const FREE_LIMIT = 50;
  const atLimit = !isPremium && products.length >= FREE_LIMIT;

  return (
    <div>
      <SectionHeader title="Inventory" subtitle={`${filtered.length} of ${products.length} products`} right={
      ...
      <div style={{ padding: "8px 18px" }}>
        ...
        <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 6 }}>Filter by date added</div>
        <DateRangeFilter from={dateFrom} to={dateTo} onFrom={setDateFrom} onTo={setDateTo}
          onClear={() => { setDateFrom(""); setDateTo(""); }} />
        <button onClick={() => setLowOnly(v => !v)} style={{
          marginTop: 8, border: lowOnly ? "1px solid var(--danger)" : "1px solid var(--border)",
          background: lowOnly ? "rgba(214,88,74,0.12)" : "var(--surface2)", color: lowOnly ? "var(--danger)" : "var(--text)",
      ...
      <div style={{ padding: "6px 18px 90px" }}>
        {filtered.length === 0 ? (
          <EmptyState icon={Package} title={products.length === 0 ? "No products yet" : "No products found"}
            subtitle={products.length === 0 ? "Add your first product to start tracking stock." : "Try a different search, date range, or filter."} />
        ) : filtered.map((p, i) => {
  ...
  return (
    <div>
      ...
      <div style={{ padding: "8px 18px" }}>
        <div style={{ position: "relative" }}>
          <Search size={15} style={{ position: "absolute", left: 12, top: 12, color: "var(--muted)" }} />
          <input className="bz-input" style={{ paddingLeft: 34 }} placeholder="Search category or note…" value={filter} onChange={e => setFilter(e.target.value)} />
        </div>
        <DateRangeFilter from={dateFrom} to={dateTo} onFrom={setDateFrom} onTo={setDateTo}
          onClear={() => { setDateFrom(""); setDateTo(""); }} />
      </div>
      <div style={{ padding: "6px 18px 90px" }}>
        {sorted.length === 0 ? (
          <EmptyState icon={Wallet} title={expenses.length === 0 ? "No expenses logged" : "No expenses found"}
            subtitle={expenses.length === 0 ? "Track supplies, rent, and other costs here." : "Try a different search or date range."} />
        ) : sorted.map((e, i) => (
...
function MoreMenu({ colors, isPremium, isAdmin, setMoreScreen, business, authUser, alertsCount = 0 }) {
  const items = [
    { id: "notifications", label: "Notifications", icon: Bell, badgeCount: alertsCount,
      desc: alertsCount > 0 ? `${alertsCount} alert${alertsCount === 1 ? "" : "s"} need attention` : "You're all caught up" },
    { id: "customers", label: "Customers", icon: Users, desc: "Manage your customer list" },
    { id: "utang", label: "Utang / Credit", icon: CreditCard, desc: "Track balances owed by customers" },
            ...
            <button key={it.id} onClick={() => setMoreScreen(locked ? "upgrade" : it.id)} className="bz-card" style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: 13.5, display: "flex", alignItems: "center", gap: 6 }}>
                  {it.label} {it.premium && !isPremium && <PremiumBadge />}
                  {!!it.badgeCount && (
                    <span style={{ background: "var(--danger)", color: "#fff", fontSize: 9.5, fontWeight: 800, borderRadius: 999, padding: "1px 6px", lineHeight: 1.4 }}>
                      {it.badgeCount > 9 ? "9+" : it.badgeCount}
                    </span>
                  )}
                </div>
...
function NotificationsScreen({ alerts, colors, onBack, onOpenStock, onOpenUtang }) {
  return (
    <div>
      <SectionHeader title="Notifications" subtitle={`${alerts.length} active alert${alerts.length === 1 ? "" : "s"}`} onBack={onBack} />
      <div style={{ padding: "10px 18px 40px" }}>
        {alerts.length === 0 ? (
          <EmptyState icon={Bell} title="You're all caught up" subtitle="No low-stock items or unpaid utang need attention right now." />
        ) : alerts.map((a, i) => {
          const Icon = a.kind === "utang" ? CreditCard : AlertTriangle;
          return (
            <Reveal key={a.id} delay={Math.min(i, 8) * 30}>
              <button onClick={() => (a.kind === "utang" ? onOpenUtang(a.customerId) : onOpenStock())} className="bz-card" style={{
                width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", marginBottom: 9,
                cursor: "pointer", textAlign: "left", border: "1px solid var(--border)",
              }}>
                <div style={{
                  width: 36, height: 36, borderRadius: 10, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
                  background: a.kind === "utang" ? "rgba(220,80,80,0.12)" : "rgba(227,162,60,0.15)",
                }}>
                  <Icon size={16} color={a.kind === "utang" ? "var(--danger)" : "var(--accent)"} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 13.5 }}>{a.title}</div>
                  <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 1 }}>{a.subtitle}</div>
                </div>
...
function CustomersScreen({ customers, sales, colors, onAdd, onDelete, onBack, goUtang }) {
  return (
    <div>
      ...
      <div style={{ padding: "10px 18px 90px" }}>
        {customers.length === 0 ? (
          ...
          const balance = purchases.reduce((a, s) => a + Math.max(0, s.total - (s.amountPaid ?? s.total)), 0);
          return (
            <Reveal key={c.id} delay={Math.min(i, 8) * 30}>
            <div className="bz-card" onClick={() => balance > 0 && goUtang && goUtang(c.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 13px", marginBottom: 8, cursor: balance > 0 ? "pointer" : "default" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {balance > 0 && (
                  <div style={{ fontSize: 11, color: "var(--danger)", fontWeight: 700, marginTop: 2 }}>Owes {peso(balance)} — tap to view</div>
                )}
              </div>
              <div className="bz-num" style={{ fontWeight: 700, fontSize: 12.5 }}>{peso(spent)}</div>
              <button onClick={(e) => { e.stopPropagation(); onDelete(c.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted)" }}><Trash2 size={13} /></button>
            </div>
            </Reveal>
          );
        })}
...
function UtangScreen({ customers, sales, colors, onBack, onRecordPayment, openCustomerId }) {
  const [expanded, setExpanded] = useState(openCustomerId || null);
  const [payAmount, setPayAmount] = useState("");

  const ledger = useMemo(() => {
    return customers.map(c => {
      const unpaid = sales
        .filter(s => s.customerId === c.id && (s.total - (s.amountPaid ?? s.total)) > 0)
        .sort((a, b) => a.date.localeCompare(b.date));
      const balance = unpaid.reduce((a, s) => a + (s.total - (s.amountPaid ?? s.total)), 0);
      return { customer: c, unpaid, balance };
    }).filter(x => x.balance > 0).sort((a, b) => b.balance - a.balance);
  }, [customers, sales]);

  const totalOwed = ledger.reduce((a, x) => a + x.balance, 0);

  const toggle = (id) => { setExpanded(prev => prev === id ? null : id); setPayAmount(""); };

  const submitPayment = (customerId, max) => {
    const amt = Math.min(Number(payAmount) || 0, max);
    if (amt <= 0) return;
    onRecordPayment(customerId, amt);
    setPayAmount("");
  };

  return (
    <div>
      <SectionHeader title="Utang / Credit" subtitle={`${ledger.length} customer${ledger.length === 1 ? "" : "s"} with balance`} onBack={onBack} />
      <div style={{ padding: "10px 18px 90px" }}>
        <div className="bz-card" style={{ padding: "14px 16px", marginBottom: 14, background: "var(--surface2)" }}>
          <div style={{ fontSize: 11.5, color: "var(--muted)", fontWeight: 600 }}>Total Outstanding</div>
          <div className="bz-num" style={{ fontSize: 22, fontWeight: 800, color: "var(--danger)", marginTop: 2 }}>{peso(totalOwed)}</div>
        </div>

        {ledger.length === 0 ? (
          <EmptyState icon={CreditCard} title="No outstanding balances" subtitle="All utang is settled. Nice!" />
        ) : ledger.map(({ customer: c, unpaid, balance }, i) => {
          const isOpen = expanded === c.id;
          return (
            <Reveal key={c.id} delay={Math.min(i, 8) * 30}>
              <div className="bz-card" style={{ padding: "12px 13px", marginBottom: 9 }}>
                <button onClick={() => toggle(c.id)} style={{
                  width: "100%", display: "flex", alignItems: "center", gap: 12, background: "none", border: "none",
                  cursor: "pointer", padding: 0, textAlign: "left", color: "var(--text)",
                }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--danger)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                    {c.name.charAt(0).toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13.5 }}>{c.name}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>{unpaid.length} unpaid sale{unpaid.length === 1 ? "" : "s"}</div>
                  </div>
                  <div className="bz-num" style={{ fontWeight: 800, fontSize: 14, color: "var(--danger)" }}>{peso(balance)}</div>
                  <ChevronRight size={15} color="var(--muted)" style={{ transform: isOpen ? "rotate(90deg)" : "none", transition: "transform .15s" }} />
                </button>

                {isOpen && (
                  <div className="bz-fade-in" style={{ marginTop: 10, paddingTop: 10, borderTop: "1px dashed var(--border)" }}>
                    {unpaid.map(s => {
                      const owed = s.total - (s.amountPaid ?? s.total);
                      return (
                        <div key={s.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 5 }}>
                          <span style={{ color: "var(--muted)" }}>{fmtDateShort(s.date)} · {s.productName} × {s.qty}</span>
                          <span className="bz-num" style={{ fontWeight: 600 }}>{peso(owed)}</span>
                        </div>
                      );
                    })}

                    <div style={{ display: "flex", gap: 7, marginTop: 10 }}>
                      <input className="bz-input" type="number" placeholder={`Amount (max ${peso(balance)})`} value={payAmount}
                        onChange={e => setPayAmount(e.target.value)} style={{ flex: 1, padding: "9px 10px", fontSize: 12.5 }} />
                      <button onClick={() => submitPayment(c.id, balance)} className="bz-btn-primary" style={{ padding: "9px 14px", fontSize: 12.5, whiteSpace: "nowrap" }}>
                        Record Payment
                      </button>
                    </div>
                    <div style={{ display: "flex", gap: 7, marginTop: 7 }}>
                      <button onClick={() => onRecordPayment(c.id, balance)} style={{
                        flex: 1, padding: "8px", borderRadius: 10, border: "1px solid var(--primary)", background: "rgba(14,110,92,0.06)",
                        color: "var(--primary)", fontSize: 12, fontWeight: 700, cursor: "pointer",
                      }}>Mark Fully Paid</button>
                      {c.phone && (
                        <a href={`sms:${c.phone}?body=${encodeURIComponent(`Hi ${c.name}, this is a friendly reminder that you have an outstanding balance of ${peso(balance)}. Thank you!`)}`}
                          style={{
                            flex: 1, padding: "8px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface2)",
                            color: "var(--text)", fontSize: 12, fontWeight: 700, cursor: "pointer", textAlign: "center", textDecoration: "none",
                          }}>Send Reminder</a>
                      )}
                    </div>
                  </div>
...
function buildReportCSV({ business, sales, expenses, totalRevenue, totalExpenses, totalProfit, inventoryValuation, bestSellers, totalUtang, utangByCustomer }) {
  const lines = [];
  ...
  lines.push(["Utang / Credit Outstanding", (totalUtang || 0).toFixed(2)].join(","));
  lines.push("");
  lines.push("Utang / Credit by Customer");
  lines.push(["Customer", "Balance Owed (PHP)"].join(","));
  (utangByCustomer || []).forEach(u => lines.push([csvEscape(u.customer.name), u.balance.toFixed(2)].join(",")));
  lines.push("");
  ...
  lines.push("");
  lines.push("Sales");
  lines.push(["Date", "Product", "Qty", "Unit Price", "Total", "Payment", "Amount Paid", "Balance Owed"].join(","));
  sales.forEach(s => {
    const paid = s.amountPaid ?? s.total;
    const owed = Math.max(0, s.total - paid);
    lines.push([s.date, csvEscape(s.productName), s.qty, s.unitPrice.toFixed(2), s.total.toFixed(2), s.payment, paid.toFixed(2), owed.toFixed(2)].join(","));
  });
  lines.push("");
  lines.push("Expenses");
...
function openPrintableReport({ business, totalRevenue, totalExpenses, totalProfit, inventoryValuation, inventoryRetailValue, bestSellers, sales, expenses, totalUtang, utangByCustomer }) {
  ...
  const rowsUtang = (utangByCustomer || []).map(u => `<tr><td>${u.customer.name}</td><td>${peso(u.balance)}</td></tr>`).join("");
  ...
  <h2>Utang / Credit Outstanding — ${peso(totalUtang || 0)}</h2>
  <table><tr><th>Customer</th><th>Balance Owed</th></tr>${rowsUtang || "<tr><td colspan=2>No outstanding balances.</td></tr>"}</table>
...
function ReportsScreen({ sales, expenses, products, customers, colors, isPremium, onBack, goUpgrade, business, showToast }) {
  ...
  // Utang / Credit: unpaid balance across all sales, plus a per-customer
  // breakdown so the report shows who owes what — not just the total.
  const totalUtang = sales.reduce((a, s) => a + Math.max(0, s.total - (s.amountPaid ?? s.total)), 0);
  const utangByCustomer = useMemo(() => {
    const map = {};
    sales.forEach(s => {
      const owed = s.total - (s.amountPaid ?? s.total);
      if (owed > 0) map[s.customerId] = (map[s.customerId] || 0) + owed;
    });
    return Object.entries(map)
      .map(([cid, balance]) => ({ customer: (customers || []).find(c => c.id === cid), balance }))
      .filter(x => x.customer)
    ...
    <div>
      ...
      <div style={{ padding: "10px 18px" }}>
        ...
        <div className="bz-card" style={{ padding: "13px 14px", marginTop: 14 }}>
          <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span>Utang / Credit Outstanding</span>
            <span className="bz-num" style={{ fontWeight: 800, color: "var(--danger)" }}>{peso(totalUtang)}</span>
          </div>
          {utangByCustomer.length === 0 ? (
            <div style={{ fontSize: 12, color: "var(--muted)" }}>No outstanding balances.</div>
          ) : utangByCustomer.map(({ customer, balance }) => (
            <Row key={customer.id} label={customer.name} value={peso(balance)} />
          ))}
        </div>
          ...
          {isPremium ? (
            <div style={{ display: "flex", gap: 10 }}>
              <button className="bz-btn-primary" style={{ flex: 1, padding: "11px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13 }}
                onClick={() => {
                  const ok = openPrintableReport({ business, totalRevenue, totalExpenses, totalProfit, inventoryValuation, inventoryRetailValue, bestSellers, sales, expenses, totalUtang, utangByCustomer });
              ...
              <button className="bz-btn-primary" style={{ flex: 1, padding: "11px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13, background: "var(--surface2)", color: "var(--text)" }}
                onClick={() => {
                  const csv = buildReportCSV({ business, sales, expenses, totalRevenue, totalExpenses, totalProfit, inventoryValuation, bestSellers, totalUtang, utangByCustomer });
...
function SettingsScreen({ business, setBusiness, theme, setTheme, colors, isPremium, authUser, onBack, goUpgrade, onLogout, onReplayTutorial, onLoadDemoData, showToast, notifyLowStock, setNotifyLowStock,
  ...
  const [pendingRestore, setPendingRestore] = useState(null); // parsed backup awaiting confirmation

  const handleDownloadBackup = async () => {
    const backup = buildBackup({ business, products, customers, sales, expenses, invoices });
    try {
      const { filename, method } = await downloadBackupFile(backup);
      if (method === "cancelled") return;
      showToast && showToast(method === "share" ? "Backup ready — choose where to save it ✓" : `Backup saved: ${filename}`);
    } catch (err) {
      showToast && showToast("Couldn't create the backup file. Please try again.");
    }
  };

  const handlePickRestoreFile = () => {
    restoreInputRef.current && restoreInputRef.current.click();
  };
  ...
  const handleRestoreFile = async (e) => {
    ...
    try {
      ...
      showToast && showToast(err.message || "Couldn't read that backup file.");
    } finally {
      setRestoring(false);
    }
  };
  ...
  const confirmRestore = () => {
    ...
    if (pendingRestore.business) { setBusiness(pendingRestore.business); setForm(pendingRestore.business); }
    setProducts(pendingRestore.products);
    setCustomers(pendingRestore.customers);
    setSales(pendingRestore.sales);
    setExpenses(pendingRestore.expenses);
    setInvoices(pendingRestore.invoices);
    setPendingRestore(null);
    showToast && showToast("Backup restored ✓");
  };
      ...
      <div style={{ padding: "10px 18px 40px" }}>
        ...
        <div className="bz-card" style={{ padding: "14px", marginTop: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0 6px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}><Bell size={15} /> Low-stock & utang alerts</div>
            {isPremium ? (
              <button onClick={() => { setNotifyLowStock(!notifyLowStock); showToast && showToast(!notifyLowStock ? "Alerts enabled" : "Alerts turned off"); }}
                style={{
          ...
          <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: -4, marginBottom: 4 }}>Shows a bell badge for low-stock items and unpaid customer utang.</div>

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0 6px", borderTop: "1px solid var(--border)", marginTop: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}><Bell size={15} /> Push notifications</div>
            <button onClick={() => onTogglePush && onTogglePush(!notifyPush)}
              style={{
                width: 40, height: 23, borderRadius: 999, border: "none", cursor: "pointer", position: "relative",
                background: notifyPush ? "var(--primary)" : "var(--surface2)", transition: "background .18s ease",
              }}>
              <div style={{
                position: "absolute", top: 2.5, left: notifyPush ? 19 : 2.5, width: 18, height: 18, borderRadius: "50%",
                background: "#fff", transition: "left .18s ease", boxShadow: "0 1px 2px rgba(0,0,0,0.25)",
              }} />
            </button>
          </div>
          <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: -4 }}>Get a phone/browser notification once a day when something needs attention.</div>
          {onReplayTutorial && (
        ...
        <div className="bz-card" style={{ padding: "14px", marginTop: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: "#4A7CB8", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Download size={19} color="#fff" />
            </div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14 }}>Backup & Restore</div>
              <div style={{ fontSize: 11.5, color: "var(--muted)" }}>Save a copy of your data, or load one back in</div>
            </div>
          </div>
          <div style={{ fontSize: 11.5, color: "var(--muted)", marginBottom: 12, lineHeight: 1.5 }}>
            {products.length} products · {sales.length} sales · {expenses.length} expenses · {customers.length} customers · {invoices.length} invoices
          </div>
          <button onClick={handleDownloadBackup} className="bz-btn-primary" style={{ width: "100%", padding: "11px", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ...
          <button onClick={handlePickRestoreFile} disabled={restoring} style={{
            width: "100%", marginTop: 8, padding: "11px", borderRadius: 14, border: "1px solid var(--border)", background: "var(--surface)",
            color: "var(--text)", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
            opacity: restoring ? 0.6 : 1,
          }}>
            <PackagePlus size={14} style={{ transform: "rotate(180deg)" }} /> {restoring ? "Reading file…" : "Restore from Backup"}
          </button>

          {pendingRestore && (
            <div className="bz-fade-in" style={{ marginTop: 12, padding: "12px", borderRadius: 12, border: "1px solid var(--accent)", background: "rgba(227,162,60,0.08)" }}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                <AlertTriangle size={13} color="var(--accent)" /> Replace current data?
              </div>
              <div style={{ fontSize: 11.5, color: "var(--muted)", marginBottom: 4 }}>
                This file has {pendingRestore.products.length} products, {pendingRestore.sales.length} sales, {pendingRestore.expenses.length} expenses, {pendingRestore.customers.length} customers, and {pendingRestore.invoices.length} invoices
                {pendingRestore.exportedAt ? `, backed up ${fmtDate(pendingRestore.exportedAt.slice(0, 10))}` : ""}.
              </div>
              <div style={{ fontSize: 11.5, color: "var(--danger)", marginBottom: 10 }}>
                It will replace everything currently in this account. This can't be undone.
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setPendingRestore(null)} style={{
                  flex: 1, padding: "10px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface)",
                  color: "var(--text)", fontSize: 12.5, fontWeight: 600, cursor: "pointer",
                }}>
                  Cancel
                </button>
                <button onClick={confirmRestore} className="bz-btn-primary" style={{ flex: 1, padding: "10px", fontSize: 12.5, background: "var(--danger)" }}>
                  Yes, Restore
                </button>
              </div>
            </div>
...
function UpgradeScreen({ colors, isPremium, authUser, business, sbToken, sbUserId, onBack, goSettings, showToast }) {
  ...
  const price = cycle === "monthly" ? 249 : 2689;
  const priceLabel = cycle === "monthly" ? "₱249" : "₱2,689";
    ...
    <div>
      ...
      <div style={{ padding: "6px 18px" }}>
        ...
        <div style={{ display: "flex", gap: 8, margin: "14px 0" }}>
          <PlanOption active={cycle === "monthly"} onClick={() => setCycle("monthly")} title="Monthly" price="₱249/mo" note="Billed monthly" />
          <PlanOption active={cycle === "yearly"} onClick={() => setCycle("yearly")} title="Yearly" price="₱2,689/yr" note="≈ ₱224/mo · billed yearly" badge="Save 10%" />
        </div>
        ...
        <div style={{ textAlign: "center", fontSize: 11, color: "var(--muted)", marginTop: 8 }}>
          Then {cycle === "monthly" ? "₱249/month" : "₱2,689/year"}. We activate Premium manually after payment is confirmed.
        </div>
      </div>
      <div style={{ height: 20 }} />
    </div>
...
// A user counts as "online" if their last heartbeat (see the heartbeat
// effect above, ~every 60s while the app is open) landed within the last
// 2 minutes — a small buffer over the 60s interval so one slightly-late
// tick doesn't flip someone to "offline" and back.
...
// Friendly relative time for "last seen" — "Online now", "5 min ago",
// "3 hr ago", falling back to a date once it's more than a day old.
const formatLastSeen = (lastSeen) => {
  if (!lastSeen) return "Never";
  if (isUserOnline(lastSeen)) return "Online now";
  ...
  return new Date(lastSeen).toLocaleDateString("en-PH");
};
  ...
  const loadAll = async () => {
    ...
    try {
      const [reqs, usrs] = await Promise.all([
        ...
        sbRest("/profiles?order=last_seen.desc.nullslast", { token: sbToken }),
      ]);
      setRequests(reqs || []);
      setUsers(usrs || []);
    } catch (e) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: "6px 18px 40px" }}>
        ...
        {tabView === "users" && users && users.length > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, margin: "0 0 12px", fontSize: 11.5, color: "var(--muted)" }}>
            <span style={{ width: 8, height: 8, borderRadius: 999, background: "var(--success)", display: "inline-block" }} />
            <span style={{ fontWeight: 700, color: "var(--text)" }}>{onlineCount}</span> active now
          </div>
        )}
          ...
          users === null ? (
            ...
            <div key={u.id} className="bz-card" style={{ padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ position: "relative" }}>
                ...
                {online && (
                  <span style={{
                    position: "absolute", bottom: -1, right: -1, width: 10, height: 10, borderRadius: 999,
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: online ? "var(--success)" : "var(--muted)", fontWeight: online ? 700 : 400, marginTop: 2 }}>
                  {formatLastSeen(u.last_seen)}
                  {!online && u.registered_at ? ` · Joined ${new Date(u.registered_at).toLocaleDateString("en-PH")}` : ""}
                </div>
              </div>
...
function AddSaleModal({ products, customers, colors, business, onClose, onSave }) {
  ...
  const total = product ? product.sellingPrice * qtyNum : 0;
  const insufficientStock = product && qtyNum > product.stock;
  ...
  return (
    <Modal title="Record a Sale" onClose={onClose} colors={colors}>
      ...
      <Field label="Quantity">
        <input className="bz-input" type="number" min={1} value={qty} onChange={e => setQty(e.target.value)} onBlur={() => setQty(q => (q === "" || Number(q) < 1) ? "1" : String(Math.floor(Number(q))))} />
      </Field>
      ...
      <button onClick={() => setIsCredit(v => !v)} className="bz-card" style={{
        width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 13px", marginBottom: 14,
        cursor: "pointer", textAlign: "left", border: isCredit ? "1.5px solid var(--danger)" : "1px solid var(--border)",
        background: isCredit ? "rgba(220,60,60,0.06)" : "var(--surface)",
      }}>
        <div style={{
          width: 20, height: 20, borderRadius: 6, border: `1.5px solid ${isCredit ? "var(--danger)" : "var(--border)"}`,
          background: isCredit ? "var(--danger)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
        }}>
          {isCredit && <Check size={13} color="#fff" />}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 12.5, color: isCredit ? "var(--danger)" : "var(--text)" }}>Utang (Pay Later)</div>
          <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 1 }}>Adds the full amount to this customer's balance instead of marking it paid now.</div>
        </div>
      </button>
      ...
      <div className="bz-card" style={{ padding: "12px 14px", marginBottom: 14, background: "var(--surface2)" }}>
        ...
        {isCredit && (
          <div style={{ fontSize: 11, color: "var(--danger)", marginTop: 4, fontWeight: 600 }}>Recorded as unpaid utang for {product ? (customers.find(c => c.id === customerId)?.name || "this customer") : "this customer"}.</div>
        )}
      </div>

      <button disabled={!product || insufficientStock || qtyNum < 1} className="bz-btn-primary" style={{ width: "100%", padding: "13px", fontSize: 14, opacity: (!product || insufficientStock || qtyNum < 1) ? 0.5 : 1 }}
        onClick={() => onSave({ date: todayISO(), productId, productName: product.name, qty: qtyNum, unitPrice: product.sellingPrice, total, customerId, payment, notes, invoiced: false, amountPaid: isCredit ? 0 : total })}>
        Save Transaction
      </button>
    </Modal>
  );
    ...
    <Modal title={product ? "Edit Product" : "Add Product"} onClose={onClose} colors={colors}>
      ...
      {mode === "catalog" ? (
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1 }}><Field label="Purchase price (₱)"><input className="bz-input" type="number" value={form.purchasePrice} onChange={e => setForm({ ...form, purchasePrice: e.target.value })} /></Field></div>
            <div style={{ flex: 1 }}><Field label="Selling price (₱)"><input className="bz-input" type="number" value={form.sellingPrice} onChange={e => setForm({ ...form, sellingPrice: e.target.value })} /></Field></div>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1 }}><Field label="Current stock"><input className="bz-input" type="number" value={form.stock} onChange={e => setForm({ ...form, stock: e.target.value })} /></Field></div>
            <div style={{ flex: 1 }}><Field label="Min stock level"><input className="bz-input" type="number" value={form.minStock} onChange={e => setForm({ ...form, minStock: e.target.value })} /></Field></div>
          </div>
