...
function BizoraMascot({ size = 60 }) {
  ...
  const [tipIndex, setTipIndex] = useState(() => Math.floor(Math.random() * MASCOT_TIPS.length));
  const autoTimerRef = useRef(null);

  // Auto-advance to the next tip every 3 seconds, looping back to the
  // start. Any manual poke (see below) resets this timer so the two never
  // fight over which tip should be showing next.
  useEffect(() => {
  ...
  const tip = MASCOT_TIPS[tipIndex];
