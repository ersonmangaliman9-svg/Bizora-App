...
const MASCOT_TIPS = [
  "Add today's sales and watch your profit grow! 📈",
  "Tip: Log expenses daily so your numbers stay honest.",
  "Low stock? I'll shout kung may kulang! 🔔",
  "Boss mode: ON. Let's check today's numbers!",
  "Every peso counted, every sale tracked. Ikaw na, boss!",
  "Ka-ching! Don't forget to record today's sales.",
  "Sipag + Bizora = walang lusot na benta 💪",
];

/* Pesoy is built as a genuine 3D object (not a flat drawing): a coin "drum"
   made of a front face, a back face, and a ring of thin rim slices placed
   around the edge with CSS 3D transforms (transform-style: preserve-3d +
   perspective). It idles with a gentle 3-axis wobble so the depth always
   reads, and a tap sends it through a real rotateY coin flip that reveals
   the rim and the back face before landing on the front again. */
function BizoraMascot({ size = 60 }) {
  const uid = useId();
  const [wave, setWave] = useState(0);
  const [tip, setTip] = useState(() => MASCOT_TIPS[Math.floor(Math.random() * MASCOT_TIPS.length)]);

  const radius = size / 2;
  const thickness = Math.max(8, Math.round(size * 0.16));
  const segments = 28;
  const segAngle = 360 / segments;
  const segWidth = Math.ceil((2 * Math.PI * radius) / segments) + 1;
  const rimIds = useMemo(() => Array.from({ length: segments }, (_, i) => i), [segments]);

  const poke = () => {
    setWave(w => w + 1);
    setTip(t => {
      let next = MASCOT_TIPS[Math.floor(Math.random() * MASCOT_TIPS.length)];
      while (next === t && MASCOT_TIPS.length > 1) next = MASCOT_TIPS[Math.floor(Math.random() * MASCOT_TIPS.length)];
      return next;
    });
  };
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <button onClick={poke} title="Kumusta! Tap me for a spin 👋" style={{ background: "none", border: "none", padding: 0, cursor: "pointer", flexShrink: 0, lineHeight: 0 }}>
        <div style={{ width: size, height: size, perspective: size * 8 }}>
          <div className="bz3d-idle" style={{ width: size, height: size, position: "relative", transformStyle: "preserve-3d" }}>
            <div key={wave} className={wave ? "bz3d-flip" : ""} style={{ width: size, height: size, position: "relative", transformStyle: "preserve-3d" }}>

              {/* rim: a ring of thin slices standing in for the coin's edge */}
              {rimIds.map(i => (
                <div key={i} style={{
                  position: "absolute",
                  width: segWidth, height: thickness,
                  left: `calc(50% - ${segWidth / 2}px)`, top: `calc(50% - ${thickness / 2}px)`,
                  background: i % 2 === 0
                    ? "linear-gradient(180deg,#E9AE55,#C97F1E)"
                    : "linear-gradient(180deg,#C97F1E,#93590F)",
                  transform: `rotateY(${i * segAngle}deg) translateZ(${radius}px)`,
                  backfaceVisibility: "hidden",
                }} />
              ))}

              {/* back face — "tails" side, shown mid-flip */}
              <div style={{ position: "absolute", inset: 0, transform: `translateZ(-${thickness / 2}px) rotateY(180deg)`, backfaceVisibility: "hidden" }}>
                <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <linearGradient id={`mBack-${uid}`} x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
                      <stop offset="0" stopColor="#14876F" />
                      <stop offset="1" stopColor="#0A5548" />
                    </linearGradient>
                  </defs>
                  <circle cx="50" cy="52" r="33" fill={`url(#mBack-${uid})`} stroke="#0A5548" strokeWidth="2" />
                  <circle cx="50" cy="52" r="33" fill="none" stroke="#F6F4EE" strokeOpacity="0.15" strokeWidth="3" />
                  <circle cx="50" cy="52" r="25" fill="none" stroke="#F6F4EE" strokeOpacity="0.3" strokeWidth="1.5" />
                  <text x="50" y="61" textAnchor="middle" fontFamily="Sora, sans-serif" fontWeight="800" fontSize="30" fill="#E3A23C" opacity="0.9">₱</text>
                </svg>
              </div>

              {/* front face — Pesoy's face, unchanged art */}
              <div style={{ position: "absolute", inset: 0, transform: `translateZ(${thickness / 2}px)`, backfaceVisibility: "hidden" }}>
                <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <linearGradient id={`mFront-${uid}`} x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
                      <stop offset="0" stopColor="#14876F" />
                      <stop offset="1" stopColor="#0A5548" />
                    </linearGradient>
                  </defs>
                  <ellipse cx="50" cy="93" rx="20" ry="4" fill="#000" opacity="0.12" />
                  <rect x="38" y="79" width="7" height="13" rx="3.5" fill="#0A5548" />
                  <rect x="55" y="79" width="7" height="13" rx="3.5" fill="#0A5548" />
                  <path d="M23 56 Q11 61 15 73" stroke="#14876F" strokeWidth="7.5" strokeLinecap="round" fill="none" />
                  <path d="M77 56 Q91 49 87 35" stroke="#14876F" strokeWidth="7.5" strokeLinecap="round" fill="none" />
                  <circle cx="50" cy="52" r="33" fill={`url(#mFront-${uid})`} stroke="#0A5548" strokeWidth="2" />
                  <circle cx="50" cy="52" r="33" fill="none" stroke="#F6F4EE" strokeOpacity="0.15" strokeWidth="3" />
                  <text x="50" y="65" textAnchor="middle" fontFamily="Sora, sans-serif" fontWeight="800" fontSize="25" fill="#E3A23C">₱</text>
                  <circle cx="39" cy="41" r="4" fill="#F6F4EE" />
                  <circle cx="61" cy="41" r="4" fill="#F6F4EE" />
                  <circle cx="40" cy="42" r="2" fill="#0A5548" />
                  <circle cx="62" cy="42" r="2" fill="#0A5548" />
                  <circle cx="30" cy="49" r="3.5" fill="#E3A23C" opacity="0.45" />
                  <circle cx="70" cy="49" r="3.5" fill="#E3A23C" opacity="0.45" />
                  <path d="M40 49 Q50 57 60 49" stroke="#F6F4EE" strokeWidth="3" strokeLinecap="round" fill="none" />
                </svg>
              </div>
            </div>
          </div>
      ...
      <div key={tip} style={{ position: "relative", background: "var(--surface2)", borderRadius: 14, padding: "9px 13px", fontSize: 12, fontWeight: 600, color: "var(--text)", flex: 1, animation: "bzfade .25s ease" }}>
        {tip}
  ...
  return (
    <div className="bz-frame" style={{
      ...
      <style>{`
        ...
        .bz3d-idle { animation: bz3dIdle 4.5s ease-in-out infinite; }
        .bz3d-flip { animation: bz3dFlip .9s cubic-bezier(.34,1.2,.4,1); }
        @keyframes bz3dIdle { 0%, 100% { transform: rotateY(-14deg) rotateX(3deg); } 50% { transform: rotateY(14deg) rotateX(-3deg); } }
        @keyframes bz3dFlip {
          0%   { transform: rotateY(0deg) rotateX(0deg) translateY(0) scale(1); }
          22%  { transform: rotateY(160deg) rotateX(6deg) translateY(-10px) scale(1.05, .95); }
          50%  { transform: rotateY(360deg) rotateX(0deg) translateY(-14px) scale(.96, 1.06); }
          78%  { transform: rotateY(520deg) rotateX(-4deg) translateY(-3px) scale(1.02, .98); }
          100% { transform: rotateY(720deg) rotateX(0deg) translateY(0) scale(1); }
        }
      ...
      <div ref={scrollRef} className="bz-scroll" style={{ height: "100svh", overflowY: "auto", paddingBottom: 88 }}
        ...
        {tab === "dashboard" && (
          <Dashboard {...{ business, authUser, colors, theme, setTheme, scrollY, todaySalesTotal, todayExpTotal, todayProfit,
            monthRevenue, monthProfit, products, lowStock, recentTx, customers, bestSellers, chartData, isPremium, notifyLowStock, alerts,
